...
export const executeDi = (params, selectors) => {
...
function getSubDiArrSortByAllTagsNo(rankDiArr) {
  ...
  rankDiArr.length > 0 && rankDiArr.map((arr, idx) => {
    newRankArr.push(getSortArrByFilter(arr, 'allTagsMatchedNo'))
  })
  return newRankArr
}
...
function getDiArrFilterByWeekTags(selectors, rankDiArr) {
  ...
  rankDiArr.length > 0 && rankDiArr.map((subArr, index) => {
    // 有绿tag的筛选逻辑
    if (weekFlag) {
      subArr.length > 1 ? formatRankDiArr.push(getSortArrByFilter(subArr, 'weekTagsMatchedNo')) :
